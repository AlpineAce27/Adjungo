    ...
    if (userType === "pilot") {
      ...
      return (
        ...
        >
          ...
          {item.reviews < 2 && (
            <td className="text-[#dc2626] font-medium">{item.reviews}</td>
          )}
          {item.reviews >= 2 && item.reviews < 3 && (
            <td className="text-[#ea580c] font-medium">{item.reviews}</td>
          )}
          {item.reviews >= 3 && item.reviews < 4 && (
            <td className="text-[#fbbf24] font-medium">{item.reviews}</td>
          )}
          {item.reviews >= 4 && item.reviews < 5 && (
            <td className="text-[#84cc16] font-medium">{item.reviews}</td>
          )}
          <td>
            <button
              onClick={() => {
                axios
                  .put(`/api/acceptApplication/${item.applicationId}`)
                  .then((response) => {
          ...
          <td>
            <button
              onClick={() => {
                axios
                  .delete(`/api/denyApplication/${item.applicationId}`)
                  .then((response) => {
                    setItems(response.data)
                  })
              }}
        ...
        <div className="flex flex-col items-center">
          ...
          {userType === "client" && (
            <div className="flex flex-col items-center">
              ...
              <p className="font-rubik text-l pb-5 w-3/4 text-center">
                Here are the applications that are waiting for your feedback.
                Each one of these applications means a pilot has applied to one
                of you're listings. The exact listing and Pilot Id are shown
                with a link on each application. The pilot's ratings has also
                been displayed to help make your decision faster!
              </p>
            </div>
          )}
        </div>
